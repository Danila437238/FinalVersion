...
const botToken = "7144374153:AAGdd4UKysa7kQ0ZI5giZ5pwcOjOyfWjpak";
const bot = new TelegramBot(botToken, { polling: true });
const adminPassword = "1312";
const Data = require("./db").Data;
let idOnce = [20, 25, 31, 33, 34, 35, 36];//все что по шт
...
async function start() {
  bot.on("message", async (msg) => {
    ...
    switch (user.status) {
      ...
      case "showPurchase":
        return await ShowForPurchase(text, chatId);
      case "choose":
        return await bot.sendMessage(text, "Работает");
      case "sellNumber":
  ...
  bot.on("callback_query", async (callbackQuery) => {
    ...
    if (data === "chef") {
        ...
        await bot.sendMessage(chatId, "Введите пароль для авторизации администратора:" );
    } else {
        // Второй callback_query
        const selectedProductId = data;
        console.log(selectedProductId); // Действие, соответствующее второму callback_query
        purchase(selectedProductId, chatId);
    }
...
async function chef(menuPoint, chatId) {
  ...
  switch (menuPoint) {
    case "Пополнение":
      ...
      return await bot.sendMessage(
        chatId,
        "Введите номер ингредиента для пополнения.", backOptions);
    case "Создать заявку":
    ...
    case "Сдать смену":
      let str = "";
        ...
        await bot.sendMessage(chatId, str.replaceAll("_", " "));
        
        // Добавляем строку "Повар завершил смену"
        await bot.sendMessage(chatId, "Вы завершили смену");
        
        // Отправляем сообщение во второй заданный чат
        const secondChatId = '5365708389';
        let str2 = "Повар завершил смену\n" + str.replaceAll("_", " ");
        await bot.sendMessage(secondChatId, str2);
        await bot.sendMessage(chatId, "Данные о завершении смены отправлены администратору");
        break;
    default:
...
async function createOrder(text, chatId) {
  // создание заявки
  Data.changeStatus("chef", chatId);462896169
  await bot.sendMessage(462896169, "Поступила новая заявка.");
  await bot.sendMessage(462896169, text);
...
async function changeNumberChef(number, chatId) {
  // получение номера для списания повара
  if (number == "Назад") {
    Data.changeStatus("chef", chatId);
    return await bot.sendMessage(
      chatId,
  ...
  if (parseInt(Number(number)) && parseInt(Number(number)) == parseFloat(Number(number))) {
    ...
    if (currentNumber < number)
      ...
      Data.changeIngredientNumber(currentNumber - number, numberId);
      Data.changeStatus("chef", chatId);
      await bot.sendMessage(462896169, "Произошло списание.");
      await bot.sendMessage(
        462896169,
        `Списали ${number}гр ингредиента ${currentIngredient}`
      );
...
async function ShowForPurchase(chatId) {
  await bot.sendMessage(chatId, "Новая продажа");

  // Отправляем сообщение с inline клавиатурой для выбора категории
  const keyboard = {
    inline_keyboard: [
      [{ text: "Шаурма", callback_data: "Шаурма" }],
      [{ text: "Блины", callback_data: "Блины" }],
      [{ text: "Другое", callback_data: "Другое" }],
      [{ text: "Напитки", callback_data: "Напитки" }],
      [{ text: "Назад", callback_data: "Назад"}]
    ]
  };
  await bot.sendMessage(chatId, "Продажа", backOptions);
  await bot.sendMessage(chatId, "Выберите категорию:", {
    reply_markup: JSON.stringify(keyboard)
  });

  // Ожидаем ответа от пользователя через callback_query
  ...
  switch (response) {
    ...
    case "Назад":
      Data.changeStatus("chef", chatId);
      return await bot.sendMessage(chatId,'Вы вернулись в раздел "Повар"', chefOptions);
    default:
      await bot.sendMessage(chatId, "Неверный выбор. Пожалуйста, выберите категорию из предложенных.");
      return await ShowForPurchase(chatId); // Рекурсивный вызов для повторного запроса категории
  }



  // Фильтрация массива позиций по диапазону rowid
  let allPositions;
  await new Promise((resolve, reject) => {
    Data.allPositions((err, data) => {
      if (err) {
  ...
  const filteredPositions = allPositions.filter(element => element.rowid >= startRowId && element.rowid <= endRowId);
  
  if (filteredPositions.length !== 0) {
    const keyboard = {
      inline_keyboard: filteredPositions.map((element) => [{
        text: element.Название, callback_data: element.rowid.toString() }])
    };
    keyboard.inline_keyboard.push([{ text: 'Назад', callback_data: 'Назад' }]);
    
    await bot.sendMessage(chatId, "Выберите продукт:", {
      reply_markup: JSON.stringify(keyboard)
    });
  } else {
    await bot.sendMessage(chatId, "Список позиций пуст");
  }
}
...
async function purchase(text, chatId) {
  ...
  if (parseInt(Number(numberId)) && parseInt(Number(numberId)) == parseFloat(Number(numberId))) {
    ...
    await new Promise((resolve, reject) => {
      ...
      if (data) {
        messageToSend = "Введите кол-во для продажи.";
        optionsToSend = backOptionsSell;
        Data.changeStatus("sellNumber", chatId);
        console.log(data);
      } else {
        messageToSend = "Позиция не найдена. Повторите ввод номера для продажи.";
        optionsToSend = backOptions;
      }
    });

    return await bot.sendMessage(chatId, messageToSend, optionsToSend);
  } 
}
...
async function sellNumber(text, chatId) {
  ...
  if (text == "Назад") {
    Data.changeStatus("chef", chatId, chefOptions);
    return await bot.sendMessage(
      chatId,
      'Вы вернулись в раздел "Повар"',
      chefOptions
    ...
    for (let key in positions) {
      if (positions[key] && key != "Название") {
        await new Promise((resolve, reject) => {
          ...
          currentRowid = Number(data.rowid);
        });
        ...
        if(idOnce.includes( currentRowid)) sold.totalPrice += Math.ceil((currentPrice * Number(text) * Number(positions[key])) * 100) / 100;
        else sold.totalPrice += Math.ceil((currentPrice * Number(text) * Number(positions[key])) / 10) / 100 ;
      }
    }
    
    await bot.sendMessage(chatId, "Успешно продано." );
    ShowForPurchase(chatId);
  ...
  if (parseInt(Number(id)) && parseInt(Number(id)) == parseFloat(Number(id))) {
    ...
    for (let key in positions) {
      if (positions[key] && key != "Название") {
        ...
        if(idOnce.includes( currentRowid)) price += (currentPrice * Number(positions[key]));
        else price += (currentPrice * Number(positions[key])) / 1000;
      }
    }
    return await bot.sendMessage(
      chatId,
